-- Runtime regression for the five-object Valorant game collection.
...
begin
  insert into auth.users (
    ...
    (
      ...
      'authenticated',
      'authenticated',
      'valorant-pack-' || v_suffix || '@example.invalid',
      pg_catalog.now(),
      '{"provider":"email","providers":["email"]}'::jsonb,
      jsonb_build_object('pseudo', 'valorant-' || left(v_suffix, 12)),
      pg_catalog.now(),
      pg_catalog.now()
    ),
    (
      ...
      'authenticated',
      'authenticated',
      'valorant-poor-' || v_poor_suffix || '@example.invalid',
      pg_catalog.now(),
      '{"provider":"email","providers":["email"]}'::jsonb,
      jsonb_build_object('pseudo', 'valorant-p-' || left(v_poor_suffix, 10)),
      pg_catalog.now(),
      pg_catalog.now()
    );
  ...
  then
    raise exception 'Valorant pack test profiles were not created';
  end if;
  ...
  select public.clutch_pack_cosmetique_v1('valorant-collection')
  into v_before;

  if (v_before ->> 'prix_volts')::integer <> 900
     or (v_before ->> 'nombre_objets')::integer <> 5
     or v_before ->> 'accent' is distinct from '#FF4655'
     or v_before ->> 'marque_key' is distinct from 'valorant'
     or jsonb_array_length(v_before -> 'objets') <> 5
     or (v_before ->> 'possede')::boolean
     or (v_before ->> 'achetable')::boolean
     or exists (
       (values
         (1, 'valorant-vandal'::text),
         (2, 'valorant-spike'),
         (3, 'valorant-jett-gallery'),
         (4, 'valorant-omen'),
         (5, 'valorant-wingman')
       )
       except
  ...
  then
    raise exception 'Valorant initial read model is inconsistent: %', v_before;
  end if;

  insert into public.volts_mouvements (user_id, montant, origine, reference)
  values (v_user, 900, 'ajustement', 'valorant-pack-test-credit');

  select public.clutch_acheter_pack_cosmetique_v1('valorant-collection')
  into v_purchase;
  select public.clutch_acheter_pack_cosmetique_v1('valorant-collection')
  into v_repeat;
  ...
  then
    raise exception 'Valorant purchase is not atomic/idempotent: first=%, repeat=%',
      v_purchase,
      v_repeat;
  end if;
  ...
  if (
    ...
    where m.user_id = v_user
      ...
      and m.reference = 'valorant-collection'
      and m.source_economique = 'achat_pack_cosmetique'
      and m.pack_id = 'valorant-collection'
      and m.montant = -900
      and m.cle_idempotence = 'achat_pack:valorant-collection'
      and m.solde_apres = 0
  ) <> 1
     or (
    select count(*)
    ...
    join public.pack_cosmetique_membres m
      on m.pack_id = 'valorant-collection'
    ...
    where e.user_id = v_user
      ...
      and e.objet_id = 'valorant-jett-gallery'
  )
     or exists (
    select 1
    from public.equipement e
  ...
  then
    raise exception 'Valorant entitlement, ledger or equipment is inconsistent';
  end if;
  ...
  then
    raise exception 'Valorant post-purchase read model is inconsistent: %', v_after;
  end if;
  ...
  insert into public.volts_mouvements (user_id, montant, origine, reference)
  values (v_poor_user, 899, 'ajustement', 'valorant-poor-test-credit');

  v_rejected := false;
  begin
    perform public.clutch_acheter_cosmetique_v1('valorant-omen');
  exception when sqlstate 'P0001' then
    v_rejected := true;
  end;

  if not v_rejected then
    raise exception 'Valorant member was individually purchasable';
  end if;

  v_rejected := false;
  begin
    perform public.clutch_acheter_pack_cosmetique_v1('valorant-collection');
  exception when sqlstate 'P0001' then
    v_rejected := true;
  end;
  ...
  then
    raise exception 'Insufficient-balance Valorant purchase was not atomic';
  end if;
  ...
  if exists (
    ...
    raise exception 'Valorant pack operations crossed into competitive state';
  end if;

  raise notice 'valorant_collection_pack_v1_ok';
end;
$$;
...
begin
  if (
    ...
    where id = 'valorant-collection'
      and prix_volts = 900
  ) <> 1
     or (
    select count(*)
    from public.pack_cosmetique_membres
    where pack_id = 'valorant-collection'
  ) <> 5
     or (
    select count(*)
    from public.inventaire_packs_cosmetiques
     ...
     or jsonb_array_length(
       public.clutch_pack_cosmetique_v1('valorant-collection') -> 'objets'
     ) <> 5
  then
    raise exception 'Valorant RLS did not expose content and hide ownership';
  end if;
end;
$$;
reset role;
