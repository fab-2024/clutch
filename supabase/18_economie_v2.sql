...
--
--  La probabilité V1 est figée à l'ouverture du marché (création du match,
--  ou au déploiement de cette migration pour les matchs déjà existants).
--  Elle provient du même Elo que le moteur actuel, mais ne porte aucune marge.
-- =====================================================================
...
/** Probabilité bornée utilisée pour le rating. */
...
/**
 * Delta serveur, identique à web/js/economy.js.
 * On arrondit la magnitude positive de la perte puis on applique le signe :
 * cela évite toute divergence JS/PostgreSQL sur les demi-entiers négatifs.
 */
...
as $$
declare
  v_p numeric := clutch_borner_proba_frags(p_proba);
begin
  if p_k <= 0 then
    raise exception 'coefficient K invalide : %', p_k using errcode = '22023';
  end if;
  if p_gagnant then
    return round(p_k * (1 - v_p))::integer;
  end if;
  return -round(p_k * v_p)::integer;
end;
$$;

/** Soft reset prévu pour le passage de saison : 1000 + 40 % de l'écart. */
create or replace function public.clutch_soft_reset_frags(p_frags integer)
returns integer
language sql
immutable
as $$
  select round(1000 + 0.4 * (p_frags - 1000))::integer;
$$;

-- ------------------------------------------ Probabilité officielle / match
...
-- Aucune policy volontairement : le client passe par les RPC dédiées.

/** Snapshot Elo -> probabilité de victoire de série au moment d'ouverture. */
...
begin
  ...
  if v_elo_a is null or v_elo_b is null then
    raise exception 'Elo introuvable pour le match %', new.id;
  end if;

  v_p_map := clutch_proba_map(v_elo_a, v_elo_b);
  select coalesce(sum(proba), 0)
    into v_p_a
  from clutch_distribution(v_p_map, new.format)
  where score_a > score_b;

  insert into matchs_scoring_frags (match_id, proba_a, proba_b, source)
  values (new.id, v_p_a, 1 - v_p_a, 'elo_v1')
...
create trigger frags_snapshot_nouveau_match
  after insert on public.matchs
  for each row execute function private.clutch_creer_snapshot_frags();

-- Backfill des matchs déjà présents au moment de la migration.
...
where not exists (
  select 1 from public.matchs_scoring_frags ms where ms.match_id = m.id
)
on conflict (match_id) do nothing;
...
create index if not exists classements_frags_saison_idx
  on public.classements_frags (saison_id, frags desc, pronostics_gagnes desc);
...
create policy classements_frags_mon_score
...
-- ------------------------------------------------ Pronostics classés
-- V1 ne classe volontairement que le vainqueur du match : c'est le marché
-- binaire dont la probabilité Elo est la plus robuste et la plus lisible.
...
create index if not exists pronostics_classes_user_saison_idx
  on public.pronostics_classes (user_id, saison_id, cree_le desc);
create index if not exists pronostics_classes_match_idx
  on public.pronostics_classes (match_id)
...
create policy pronostics_classes_mes_pronos
  on public.pronostics_classes
  for select
  to authenticated
  using ((select auth.uid()) = user_id);
...
declare
  ...
  v_pb numeric;
begin
  select id, saison_id, statut, debut
    into v_match
  from matchs
  where id = p_match_id;
  ...
  if v_user is not null then
    ...
    where user_id = v_user
      and saison_id = v_match.saison_id
      and statut <> 'annule';
  end if;

  v_k := case when v_nb < clutch_frags_nb_placements()
              then clutch_frags_k_placement()
              else clutch_frags_k() end;
  v_pa := clutch_borner_proba_frags(v_snap.proba_a);
  v_pb := clutch_borner_proba_frags(v_snap.proba_b);

  return jsonb_build_object(
    'match_id', p_match_id,
    'figee_le', v_snap.figee_le,
    'source', v_snap.source,
    'k', v_k,
    'placements_restants', greatest(0, clutch_frags_nb_placements() - v_nb),
    'choix', jsonb_build_array(
      jsonb_build_object(
        'cle', 'a', 'proba', v_snap.proba_a, 'proba_scoring', v_pa,
        'gain', clutch_delta_frags(v_pa, true, v_k),
        'perte', clutch_delta_frags(v_pa, false, v_k)
      ),
      jsonb_build_object(
        'cle', 'b', 'proba', v_snap.proba_b, 'proba_scoring', v_pb,
        'gain', clutch_delta_frags(v_pb, true, v_k),
        'perte', clutch_delta_frags(v_pb, false, v_k)
      )
    )
  );
end;
...
-- ===================================================== Poser un prono
create or replace function public.placer_pronostic_classe(
  p_match_id text,
  p_choix text
)
returns jsonb
...
declare
  ...
  v_proba numeric;
  v_scoring numeric;
  v_prono pronostics_classes%rowtype;
begin
  if v_user is null then
    raise exception 'Connecte-toi pour pronostiquer.' using errcode = '28000';
  end if;
  if p_choix not in ('a', 'b') then
    raise exception 'Choix classe invalide.' using errcode = '22023';
  end if;
  ...
  if v_match.statut <> 'a_venir' or v_match.debut <= now() then
    raise exception 'Les pronostics sont fermes sur ce match.';
  end if;
  if (select statut from v_saisons where id = v_match.saison_id) <> 'en_cours' then
    raise exception 'Cette saison n''est pas ouverte aux pronostics.';
  end if;

  -- Sérialise les validations d'un même joueur : exactement 5 placements K=60.
  perform pg_advisory_xact_lock(hashtextextended(v_user::text || '|' || v_match.saison_id, 0));

  if exists (
    select 1 from pronostics_classes
    where user_id = v_user and match_id = p_match_id
  ) then
    raise exception 'Tu as deja un pronostic classe sur ce match.' using errcode = '23505';
  end if;
  ...
  insert into classements_frags (saison_id, user_id, frags, pic_frags)
  values (v_match.saison_id, v_user, clutch_frags_initial(), clutch_frags_initial())
  on conflict (saison_id, user_id) do nothing;

  insert into pronostics_classes (
    user_id, match_id, saison_id, choix,
    proba_figee, proba_scoring, k_frags
  ) values (
    v_user, p_match_id, v_match.saison_id, p_choix,
    v_proba, v_scoring, v_k
  )
  returning * into v_prono;

  return to_jsonb(v_prono) || jsonb_build_object(
    'gain_si_correct', clutch_delta_frags(v_scoring, true, v_k),
    'perte_si_faux', clutch_delta_frags(v_scoring, false, v_k),
    'placement', v_k = clutch_frags_k_placement()
  );
end;
$$;
...
begin
  ...
  for r in
    select * from pronostics_classes
    where match_id = new.id and statut = 'en_cours'
    order by cree_le, id
    for update
  loop
    ...
    insert into classements_frags (saison_id, user_id, frags, pic_frags)
    values (r.saison_id, r.user_id, clutch_frags_initial(), clutch_frags_initial())
    on conflict (saison_id, user_id) do nothing;

    select frags into v_frags_avant
    from classements_frags
    where saison_id = r.saison_id and user_id = r.user_id
    for update;

    v_frags_apres := v_frags_avant + v_delta;

    update classements_frags
...
begin
  ...
  select * into v_c
  from classements_frags
  where saison_id = p_saison_id and user_id = v_user;

  select count(*)::integer into v_places
  from pronostics_classes
  where saison_id = p_saison_id and user_id = v_user and statut <> 'annule';

  return jsonb_build_object(
    'saison_id', p_saison_id,
    'frags', coalesce(v_c.frags, clutch_frags_initial()),
    'pic_frags', coalesce(v_c.pic_frags, clutch_frags_initial()),
    'pronostics_regles', coalesce(v_c.pronostics_regles, 0),
    'pronostics_gagnes', coalesce(v_c.pronostics_gagnes, 0),
    'placements_restants', greatest(0, clutch_frags_nb_placements() - v_places),
    'provisoire', v_places < clutch_frags_nb_placements()
  );
end;
$$;
...
as $$
  select
    p.id, p.match_id, p.saison_id, p.choix,
    p.proba_figee, p.proba_scoring, p.k_frags,
    p.statut, p.delta_frags, p.cree_le, p.regle_le,
    m.equipe_a, m.equipe_b, m.score_a, m.score_b, m.debut
  from pronostics_classes p
...
-- ---------------------------------------------------------- Permissions
-- Les tables sont en lecture RLS seulement ; toutes les écritures passent
-- par les fonctions ci-dessus. Les SECURITY DEFINER exposées vérifient
-- auth.uid() et n'acceptent jamais un user_id fourni par le navigateur.
do $$
declare r text;
begin
  ...
  if exists (select 1 from pg_roles where rolname = 'authenticated') then
    ...
    -- Projection sans écriture : utile pour afficher +gain / -perte.
    grant execute on function public.clutch_projection_match_frags(text) to authenticated;
  end if;
end $$;

-- ---------------------------------------------------------------------
-- IMPORTANT : on ne supprime ni `paris`, ni `participations.solde`, ni les
-- historiques de Volts. La bascule finale pourra les archiver quand toutes
-- les vues utiliseront les RPC V2. Cette migration ne transforme donc aucune
-- donnée économique historique et reste déployable sans perte.
-- =====================================================================
