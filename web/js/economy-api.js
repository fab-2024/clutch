 ...
 * Le serveur Supabase est l'autorité : le client ne calcule jamais le rating.
 */
...
async function rpc(nom, args = {}) {
  ...
  try {
    reponse = await fetch(`${BASE}/rest/v1/rpc/${nom}`, {
      ...
      headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${jeton}`, 'Content-Type': 'application/json' },
...
async function saisonIdOuCourante(saisonId) {
  return saisonId || (await api.saisonCourante())?.id || null;
}
...
export async function classementLigueFrags(ligueId, saisonId = null) {
  const id = await saisonIdOuCourante(saisonId);
  return id ? (await rpc('clutch_classement_ligue_frags', { p_ligue_id: ligueId, p_saison_id: id })) ?? [] : [];
}
