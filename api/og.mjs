...
export async function GET(request) {
  ...
  let card = null;
  try {
    if (['challenge','match','league'].includes(kind)) {
      const data = await loadPublicObject(kind, ref);
      card = data ? publicPresentation(kind, data) : null;
    }
  } catch (error) { console.error('[public] og data error', error?.message || error); }
  if (!card) card = genericCard();
  return new ImageResponse(render(card), { width:1200, height:630, headers:{'Cache-Control':'public, s-maxage=60, stale-while-revalidate=300'} });
}
...
function render(card) {
  ...
  const detail = card.kind === 'challenge' && card.chosenTag
    ...
    : card.kind === 'league'
      ? `${card.members} membres · Leader ${card.leader} · ${Number(card.leaderFrags||1000).toLocaleString('fr-FR')} Frags`
...
glowB:{position:'absolute',width:'600px',height:'600px',borderRadius:'999px',right:'-260px',bottom:'-330px',background:'radial-gradient(circle, rgba(124,92,255,.13), rgba(124,92,255,0) 68%)'},
frame:{position:'relative',width:'100%',height:'100%',display:'flex',flexDirection:'column',border:'1px solid rgba(232,255,61,.22)',borderRadius:'34px',padding:'34px 40px',background:'linear-gradient(145deg, rgba(255,255,255,.055), rgba(255,255,255,.012))'},
top:{display:'flex',alignItems:'center',justifyContent:'space-between'},brand:{display:'flex',fontSize:'31px',fontWeight:900,letterSpacing:'-1.8px'},dot:{color:'#e8ff3d'},badge:{display:'flex',padding:'8px 13px',borderRadius:'999px',border:'1px solid rgba(232,255,61,.23)',color:'#e8ff3d',background:'rgba(232,255,61,.055)',fontSize:'13px',fontWeight:800,letterSpacing:'1.7px'},
content:{flex:1,display:'flex',flexDirection:'column',justifyContent:'center'},kicker:{color:'#e8ff3d',fontSize:'14px',fontWeight:900,letterSpacing:'2.3px',marginBottom:'10px'},headline:{fontSize:'67px',lineHeight:.92,fontWeight:900,letterSpacing:'-3.4px',maxWidth:'1020px',marginBottom:'30px'},matchup:{display:'flex',alignItems:'center',gap:'22px',width:'100%'},team:{flex:1,minWidth:0,display:'flex',flexDirection:'column',justifyContent:'center',padding:'21px 25px',borderRadius:'20px',border:'1px solid rgba(255,255,255,.095)',background:'rgba(0,0,0,.16)'},tag:{color:'#e8ff3d',fontSize:'49px',lineHeight:.9,fontWeight:900,letterSpacing:'-1.4px'},teamName:{color:'#9da69a',fontSize:'15px',marginTop:'8px',maxWidth:'390px'},vs:{display:'flex',color:'#f4f6f0',fontSize:'24px',fontWeight:900,letterSpacing:'1px',whiteSpace:'nowrap'},bottom:{display:'flex',alignItems:'center',justifyContent:'space-between',borderTop:'1px solid rgba(255,255,255,.08)',paddingTop:'20px'},detail:{color:'#9ba398',fontSize:'17px',fontWeight:700,maxWidth:'760px'},cta:{color:'#e8ff3d',fontSize:'14px',fontWeight:900,letterSpacing:'1.6px'}
};
