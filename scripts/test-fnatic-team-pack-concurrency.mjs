...
if (!/^[a-z0-9][a-z0-9-]{1,63}$/.test(packId)) {
  throw new Error(`Invalid team-pack id: ${packId}`);
}

const expectedPrice = Number.parseInt(expectedPriceArgument, 10);
const expectedMembers = Number.parseInt(expectedMembersArgument, 10);
const expectedDefaults = Number.parseInt(expectedDefaultsArgument, 10);
if (
  !Number.isSafeInteger(expectedPrice)
  || expectedPrice <= 0
  || !Number.isSafeInteger(expectedMembers)
  || expectedMembers <= 0
  || !Number.isSafeInteger(expectedDefaults)
  || expectedDefaults <= 0
  || expectedDefaults > expectedMembers
) {
  throw new Error('Invalid team-pack concurrency expectations.');
}

const startingBalance = expectedPrice + 80;
const userId = randomUUID();
const suffix = userId.replaceAll('-', '');
const firstApplicationName = `team-pack-A-${suffix.slice(0, 12)}`;
const secondApplicationName = `team-pack-B-${suffix.slice(0, 12)}`;
const barrierTable = `private.team_pack_barrier_${suffix.slice(0, 12)}`;
...
insert into auth.users (
  ...
  'authenticated',
  'authenticated',
  'team-pack-concurrency-${suffix}@example.invalid',
  pg_catalog.now(),
  '{"provider":"email","providers":["email"]}'::jsonb,
  jsonb_build_object('pseudo', 'team-c-${suffix.slice(0, 14)}'),
  pg_catalog.now(),
  pg_catalog.now()
);

insert into public.volts_mouvements (user_id, montant, origine, reference)
values ('${userId}', ${startingBalance}, 'ajustement', 'team-pack-concurrency-credit');
`;
...
$$;
select public.clutch_acheter_pack_cosmetique_v1('${packId}');
commit;
`;
...
) then 'TEAM_PACK_A_BARRIER_READY' else 'TEAM_PACK_A_BARRIER_PENDING' end;
`;
...
) then 'TEAM_PACK_B_WAIT_OVERLAP' else 'TEAM_PACK_B_WAIT_PENDING' end;
`;
...
begin
  if (
    ...
    where m.user_id = '${userId}'
      ...
      and m.reference = '${packId}'
      and m.montant = -${expectedPrice}
  ) <> 1
     or (
    select count(*)
    from public.inventaire_packs_cosmetiques i
    where i.user_id = '${userId}'
      and i.pack_id = '${packId}'
  ) <> 1
     or (
    select count(*)
  ...
  then
    raise exception 'Concurrent team-pack purchases were not idempotent: ${packId}';
  end if;
end;
$$;
`;
...
try {
  await execute(`${packId} concurrency setup`, setupSql);
  setupComplete = true;

  const firstSession = execute(`${packId} concurrent purchase A`, firstPurchaseSql);
  activeSessions.push(firstSession);
  firstBarrierObserved = await waitForMarker({
    label: `${packId} A barrier probe`,
    marker: 'TEAM_PACK_A_BARRIER_READY',
    sql: firstBarrierSql,
  });

  const secondSession = execute(`${packId} concurrent purchase B`, secondPurchaseSql);
  activeSessions.push(secondSession);
  overlapObserved = await waitForMarker({
    label: `${packId} B advisory-wait probe`,
    marker: 'TEAM_PACK_B_WAIT_OVERLAP',
    sql: overlapBarrierSql,
  });

  await execute(`${packId} release A barrier`, releaseBarrierSql);
  ...
  await execute(`${packId} concurrency assertions`, assertionSql);
  ...
  console.log(JSON.stringify({
    ...
    overlapObserved,
    expectedDebits: 1,
    expectedInventoryMembers: expectedMembers,
    expectedEquippedDefaults: expectedDefaults,
    expectedBalance: 80,
  }));
} finally {
  if (setupComplete) {
    await execute(`${packId} ensure barrier release`, releaseBarrierSql).catch(() => undefined);
  }
  await Promise.allSettled(activeSessions);
  if (setupComplete) {
    await execute(`${packId} concurrency cleanup`, cleanupSql);
  }
}
