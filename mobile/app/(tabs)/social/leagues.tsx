...
import { createLeague, joinLeague, loadLeagues, type LeagueSummary } from '@/src/services/social';
...
export default function LeaguesScreen() {
  ...
  const load = useCallback(async (refresh = false) => {
    ...
    setError(null);
    try { setLeagues(await loadLeagues()); }
    catch (caught) { setError(caught instanceof Error ? caught.message : 'Impossible de charger tes ligues.'); }
  ...
  return (
    ...
    >
      <View style={styles.intro}>
        <Text style={styles.eyebrow}>LIGUES PRIVÉES</Text>
        <Text style={styles.title}>Tes mini-compétitions.</Text>
        <Text style={styles.subtitle}>Classement en Frags, rivalités entre potes et une cible claire à dépasser.</Text>
      </View>

      <View style={styles.summary}>
        <View><Text style={styles.summaryValue}>{loading ? '—' : leagues.length}</Text><Text style={styles.summaryLabel}>LIGUE{leagues.length > 1 ? 'S' : ''}</Text></View>
        <View style={styles.summaryDivider} />
        <View><Text style={styles.summaryValue}>{loading ? '—' : leagues.reduce((sum, league) => sum + league.nb_membres, 0)}</Text><Text style={styles.summaryLabel}>MEMBRES CUMULÉS</Text></View>
      </View>
      ...
      <View style={styles.list}>
        {loading ? <Skeleton /> : leagues.length ? leagues.map((league, index) => <LeagueCard key={league.id} league={league} index={index} />) : (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyTitle}>Ton premier classement commence ici.</Text>
            <Text style={styles.emptyText}>Crée une ligue ou entre le code d’un pote. Aucun Frag n’est dépensé pour participer.</Text>
          </View>
        )}
      </View>
...
function Skeleton() {
  return <View style={styles.skeleton}>{[0, 1].map((item) => <View key={item} style={styles.skeletonRow} />)}</View>;
}
...
const styles = StyleSheet.create({
  ...
  eyebrow: { color: colors.volt, fontSize: 9, fontWeight: '900', letterSpacing: 1.4 },
  title: { color: colors.text, fontSize: 31, lineHeight: 34, fontWeight: '900', letterSpacing: -1.1 },
  subtitle: { color: colors.textMuted, fontSize: 13, lineHeight: 19 },
  summary: { minHeight: 82, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 18, borderRadius: radius.lg, backgroundColor: '#0B1015', borderWidth: 1, borderColor: colors.border },
  summaryValue: { color: colors.text, fontSize: 24, fontWeight: '900' },
  summaryLabel: { marginTop: 2, color: colors.textMuted, fontSize: 7, fontWeight: '900', letterSpacing: 1 },
  summaryDivider: { width: 1, height: 38, backgroundColor: colors.border, marginHorizontal: 24 },
  ...
  cardArrow: { width: 32, height: 32, borderRadius: 16, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surfaceElevated },
  cardArrowText: { color: colors.textMuted, fontSize: 14 },
  empty: { alignItems: 'center', padding: 26, borderRadius: radius.lg, backgroundColor: '#0A0F14', borderWidth: 1, borderColor: colors.border, gap: 8 },
  ...
  skeleton: { gap: 9 },
  skeletonRow: { height: 74, borderRadius: radius.lg, backgroundColor: '#10161D' },
});
