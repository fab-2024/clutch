/**
 * Client RPC Economy V2. Supabase est l'autorité du rating Frags.
 */
...
async function saison(id) {
  return id ? { id } : await api.saisonCourante();
}

export async function etatFrags(id = null) {
  const s = await saison(id);
  return s?.id ? rpc('clutch_etat_frags', { p_saison_id: s.id }) : null;
}
...
export async function mesPronosticsClasses(id = null) {
  const s = await saison(id);
  return s?.id ? (await rpc('clutch_mes_pronostics_classes', { p_saison_id: s.id })) ?? [] : [];
}

export async function classementFrags(id = null) {
  const s = await saison(id);
  return s?.id ? (await rpc('clutch_classement_frags', { p_saison_id: s.id })) ?? [] : [];
}

export async function classementLigueFrags(ligueId, id = null) {
  const s = await saison(id);
  return s?.id
    ? (await rpc('clutch_classement_ligue_frags', { p_ligue_id: ligueId, p_saison_id: s.id })) ?? []
    : [];
}

export async function rivaliteFrags({ saisonId = null, ligueId = null } = {}) {
  const s = await saison(saisonId);
  return s?.id ? rpc('clutch_rivalite_frags', { p_saison_id: s.id, p_ligue_id: ligueId }) : null;
}
