...
begin
  ...
  if exists (select 1 from public.packs_cosmetiques where id = 'mythes-forge' and (actif or statut_publication <> 'retire')) then
    raise exception 'Collection must no longer be sold as a bundle';
  end if;
  begin
    perform public.clutch_acheter_pack_cosmetique_v1('mythes-forge');
  ...
  values (v_user, 3000, 'ajustement', 'mythes-forge-individual-test-credit');
  select public.clutch_boutique_cosmetique_v1() into v_shop;
  if (select count(*) from jsonb_array_elements(v_shop -> 'objets') o where o ->> 'collection_key' = 'mythes-forge') <> 8 then
    raise exception 'Shop must expose eight retained objects only';
  end if;
  for v_item in select * from public.objets_catalogue where collection_key = 'mythes-forge' and actif loop
    v_count := v_count + 1;
