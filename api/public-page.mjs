...
export default async function handler(req, res) {
  ...
  let data = null;
  try {
    data = kind ? await loadPublicObject(kind, ref) : null;
  } catch (error) {
    console.error('[phase9] public page data error', error?.message || error);
  }
...
function renderCardPage({ card, canonical, ogImage }) {
  ...
  const score = card.status === 'termine' && card.scoreA != null && card.scoreB != null
    ...
    : `<div class="versus"><div><b>${escapeHtml(card.tagA)}</b><span>${escapeHtml(card.equipeA)}</span></div><i>VS</i><div><b>${escapeHtml(card.tagB)}</b><span>${escapeHtml(card.equipeB)}</span></div></div>`;

  const challengeDetail = card.kind === 'challenge' && card.chosenTag
...
<head>
  ...
  <meta name="description" content="${escapeAttr(card.description)}" />
  ...
  <meta property="og:image:alt" content="${escapeAttr(`${card.tagA} vs ${card.tagB} sur Clutch`)}" />
...
function formatDate(value) {
  ...
  return new Intl.DateTimeFormat('fr-FR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit', timeZone: 'Europe/Paris' }).format(d).replace(',', ' ·');
}
