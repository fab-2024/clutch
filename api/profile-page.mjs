...
<meta property="og:type" content="profile"><meta property="og:site_name" content="GRIFF"><meta property="og:locale" content="fr_FR"><meta property="og:url" content="${attr(canonical)}"><meta property="og:title" content="${attr(profile.title)}"><meta property="og:description" content="${attr(profile.description)}"><meta property="og:image" content="${attr(ogImage)}"><meta property="og:image:width" content="1200"><meta property="og:image:height" content="630"><meta property="og:image:alt" content="Profil GRIFF de ${attr(profile.pseudo)}">
...
<body><div class="glow"></div><header><a href="/" class="brand">GRIFF<span>.</span></a><b>PUBLIC PROFILE</b></header><main><article class="card"><div class="sig"><span>${esc(profile.styleSymbol)}</span><strong>${esc(profile.style)}</strong></div><h1>${esc(profile.pseudo)}</h1><p>${esc(profile.styleText)}</p><div class="line">${esc(profile.short)}</div><div class="stats"><div><small>FRAGS</small><strong>${Number(profile.frags).toLocaleString('fr-FR')}</strong></div><div><small>RANG</small><strong>${esc(rank)}</strong></div><div><small>PRÉCISION</small><strong>${Number(profile.precision)} %</strong></div><div><small>FORME</small><strong>${esc(serie)}</strong></div></div>${profile.faction ? `<div class="faction"><small>FACTION</small><strong>${esc(profile.faction)}</strong><span>${esc(profile.factionName || '')}</span></div>` : ''}${storeLinks()}<small class="foot">Frags = classement compétitif · aucun argent réel en jeu.</small></article></main><footer><span>GRIFF // PLAYER IDENTITY</span><a href="/">Découvrir GRIFF</a></footer></body></html>`;
}

function renderMissing({ canonical, ogImage }) {
  return `<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Profil indisponible | GRIFF</title><meta name="robots" content="noindex,nofollow"><link rel="canonical" href="${attr(canonical)}"><meta property="og:title" content="GRIFF"><meta property="og:description" content="Ce profil est privé ou introuvable."><meta property="og:image" content="${attr(ogImage)}"><style>${css()}</style></head><body><header><a href="/" class="brand">GRIFF<span>.</span></a></header><main><article class="card"><div class="sig">PROFIL PUBLIC</div><h1>PROFIL INDISPONIBLE.</h1><p>Ce joueur a peut-être rendu son profil privé ou changé de pseudo.</p><a class="cta" href="/">DÉCOUVRIR GRIFF <span>›</span></a></article></main></body></html>`;
}

function storeLinks() {
  return '<div><a class="cta" href="/api/download?platform=ios">OUVRIR SUR IPHONE <span>›</span></a><a class="cta" href="/api/download?platform=android">OUVRIR SUR ANDROID <span>›</span></a></div>';
}
