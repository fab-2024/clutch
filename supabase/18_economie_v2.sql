...
returns numeric language sql immutable set search_path = '' as $$ select 0.15::numeric $$;

create or replace function public.clutch_frags_proba_max()
returns numeric language sql immutable set search_path = '' as $$ select 0.85::numeric $$;
...
begin
  ...
  return least(public.clutch_frags_proba_max(), greatest(public.clutch_frags_proba_min(), p_proba));
end;
$$;
...
set search_path = ''
as $$
declare v_p numeric := public.clutch_borner_proba_frags(p_proba);
begin
...
as $$ select round(1000 + 0.4 * (p_frags - 1000))::integer $$;
...
from public.matchs m join public.equipes ea on ea.id=m.equipe_a_id join public.equipes eb on eb.id=m.equipe_b_id
cross join lateral (
  select coalesce(sum(d.proba),0)::numeric as p_a
  from public.clutch_distribution(public.clutch_proba_map(ea.elo,eb.elo),m.format) d where d.score_a>d.score_b
) x
...
begin
  ...
  v_k:=case when v_nb<clutch_frags_nb_placements() then clutch_frags_k_placement() else clutch_frags_k() end;
  v_proba:=case when p_choix='a' then v_snap.proba_a else v_snap.proba_b end; v_scoring:=clutch_borner_proba_frags(v_proba);
