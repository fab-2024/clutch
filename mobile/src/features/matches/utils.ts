export function gameLabel(game: string) {
  ...
  if (key.includes('rocket') || key === 'rl') return 'RL';
...
export function gameKey(game: string) {
  ...
  return label === 'LoL' || label === 'VALORANT' || label === 'RL' ? label : 'Autres';
}

export { classifyMatchPhase as matchPhase, type MatchPhase } from '@/src/utils/matchStatus';
