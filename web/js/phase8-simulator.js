...
function invitation() {
  return shell(`<div class="phase8-hero phase8-hero--invite"><div class="phase8-eyebrow">INVITATION 1V1 · SIMULATION</div><h1>FABTHETAP TE DÉFIE.</h1><p>FabTheTap a pris <strong>VIT</strong> · Fort. Si tu acceptes, tu défends <strong>KC</strong>.</p><div class="phase8-matchline"><span>VIT</span><strong>VS</strong><span>KC</span><small>RL · RLCS MAJOR · 20:00</small></div><div class="phase8-versus phase8-versus--invite">${joueur('FabTheTap','VIT','Team Vitality','fort')}<div class="phase8-vs">VS</div>${joueur('TOI','KC','Karmine Corp',conviction,true,true)}</div><div class="phase8-conviction-block"><div><small>TA CONVICTION</small><strong>À quel point tu assumes ce contre-call ?</strong></div><div class="phase8-convictions">${convictions()}</div></div><button class="phase8-btn phase8-btn--accept" type="button" data-phase8-sim-state="locked">Prendre KC et verrouiller le duel</button><small class="phase8-auth-note">Simulation uniquement · aucun prono n’est créé.</small></div>`);
}

function locked() {
  return shell(`<div class="phase8-hero phase8-hero--locked"><div class="phase8-eyebrow">MATCH-UP CONFIRMÉ · SIMULATION</div><h1>DUEL VERROUILLÉ.</h1><p>RL · RLCS Major · 20:00</p><div class="phase8-versus">${joueur('FabTheTap','VIT','Team Vitality','fort')}<div class="phase8-vs">VS</div>${joueur('TOI','KC','Karmine Corp',conviction,true,true)}</div><div class="phase8-lock-note">Les deux pronostics sont verrouillés. Aucun Frag supplémentaire n’est mis en jeu par le duel.</div><div class="phase8-actions"><button class="phase8-btn" type="button" data-phase8-sim-state="win">Simuler ma victoire</button><button class="phase8-btn phase8-btn--ghost" type="button" data-phase8-sim-state="loss">Simuler ma défaite</button></div></div>`);
}

function resultat(gagne) {
  const pseudo = 'FabTheTap';
  return shell(`<div class="phase8-hero phase8-hero--locked is-finished"><div class="phase8-eyebrow">VERDICT FINAL · SIMULATION</div><h1>${gagne ? 'TU PRENDS LE DUEL.' : `${pseudo.toUpperCase()} PREND LE DUEL.`}</h1><p>RL · RLCS Major</p><div class="phase8-final-score"><span>VIT</span><strong>${gagne ? '2 — 3' : '3 — 2'}</strong><span>KC</span></div><div class="phase8-versus">${joueur(pseudo,'VIT','Team Vitality','fort')}<div class="phase8-vs">VS</div>${joueur('TOI','KC','Karmine Corp',conviction,true,true)}</div><div class="phase8-rivalry"><small>RIVALITÉ</small><strong>Toi ${gagne ? 3 : 2} — ${gagne ? 2 : 3} ${pseudo}</strong></div><div class="phase8-actions"><button class="phase8-btn" type="button" data-phase8-sim-state="invite">Rejouer</button><a class="phase8-btn phase8-btn--ghost" href="#/matchs">Fermer</a></div></div>`);
}
