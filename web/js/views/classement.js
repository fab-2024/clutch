...
export async function vueClassement(racine, { entete = true } = {}) {
  if (!contexte.utilisateur) {
    racine.innerHTML = `
      ...
      <div class="carte"><h3>Connecte-toi pour entrer dans le classement.</h3><p>Les Frags sont ton rating compétitif saisonnier.</p><a class="btn" href="#/connexion">Jouer</a></div>`;
    return;
  }
  ...
  racine.innerHTML = `
    ${
      entete
        ? `<div class="entete-page">
             <h1>Classement</h1>
             <p>${esc(contexte.saison?.nom ?? '')} — les Frags mesurent uniquement la performance de pronostic.</p>
           </div>
           ${bandeauSaison()}`
        : ''
    }
    ...
    <div class="carte" id="zone-classement">
      ${
        lignes.length
          ? tableauClassement(lignes)
          : vide('Classement vierge', 'Le premier pronostic classé réglé lancera la saison.')
      }
    </div>`;
}
  ...
  return `
    <div class="carte carte--rivalite">
      ...
      <p style="color:var(--texte-faible);font-size:0.78rem;margin:14px 0 0;text-align:center">
        ${devant
          ? `Tu mènes de ${esc(frags(Math.abs(r.ecart)))} Frags.`
          : `Il te devance de ${esc(frags(Math.abs(r.ecart)))} Frags. Nouveau rival lundi.`}
      </p>
    </div>`;
}
        ...
        ${lignes.map((l) => {
          ...
          return `<tr${l.moi ? ' class="moi"' : ''}>
            ...
            <td>
              ...
              ${l.provisoire ? ' <span class="badge">placement</span>' : ''}
            </td>
