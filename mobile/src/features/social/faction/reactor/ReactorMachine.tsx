...
import Svg, { ClipPath, Defs, Ellipse, FeColorMatrix, Filter, G, Image as SvgImage, Mask, Path, Rect } from 'react-native-svg';
...
type Props = Clocks & { accent?: string; focusModule?: boolean; active?: boolean; forge?: boolean; from: ReactorStage; to: ReactorStage };
type Piece = 'core' | 'left' | 'right' | 'base-left' | 'base-right' | 'rear-left' | 'rear-right';


function MachinePiece({ stage, piece, from, to, id, size, forge = false, accent = '#30A9FF', ...clocks }: Props & { stage: ReactorStage; piece: Piece; id: string; size: number }) {
  ...
  return <Animated.View style={[StyleSheet.absoluteFill, { transformOrigin: 'top left' }, poseStyle]}>
    ...
    <Svg width="100%" height="100%" viewBox="0 0 1000 1000" style={StyleSheet.absoluteFill}>
      ...
      <G clipPath={`url(#${clip})`}>
        ...
        <Surface cx={stage === 2 ? 620 : 500} cy={stage === 1 ? 682 : stage === 2 ? 732 : stage === 3 ? 715 : stage === 4 ? 755 : 794} rx={stage === 2 ? 34 : 38} ry="3" fill={accent} animatedProps={core} />
        {art.pipes.map((d, i) => <Light key={i} d={d} fill="none" stroke={accent} strokeWidth="4.5" strokeLinecap="round" strokeDasharray="40 1200" animatedProps={pipeLight} />)}
      </G>
    </Svg>
...
const PiecePhoto = memo(function PiecePhoto({ stage, id, area }: { stage: ReactorStage; id: string; area: number[] }) {
  const art = MACHINE_ART[stage];
    ...
    <Svg width="100%" height="100%" viewBox="0 0 1000 1000">
      <Defs>
        ...
        <Mask id={`${id}-photo`} x="0" y="0" width="1000" height="1000" maskUnits="userSpaceOnUse">
          ...
          {stage !== 1 && art.cavities.map((c, i) => <Rect key={i} x={c.x} y={c.y} width={c.width} height={c.height} rx={c.lip} fill="black" />)}
        </Mask>
        <Filter id={`${id}-steel`} x="0" y="0" width="100%" height="100%"><FeColorMatrix type="saturate" values="0.08" /></Filter>
      </Defs>
      <G clipPath={`url(#${id}-area)`}><G mask={`url(#${id}-photo)`}><SvgImage filter={`url(#${id}-steel)`} href={art.source} width="1000" height="1000" /></G></G>
    </Svg>
  </View>;
...
export function ReactorMachine(props: Props) {
  ...
  if (props.focusModule) return <FocusedModule accent={props.accent} fill={props.fill} mutation={props.mutation} reaction={props.reaction} reduced={props.reduced} />;
