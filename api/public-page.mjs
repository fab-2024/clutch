...
<body><div class="ambient" aria-hidden="true"><i></i><i></i></div><header><a class="brand" href="/">CLUTCH<span>.</span></a><span class="public">PUBLIC LINK</span></header><main><article class="card"><div class="eyebrow">${escapeHtml(card.eyebrow)}</div><h1>${escapeHtml(card.headline)}</h1><p>${escapeHtml(card.description)}</p><div class="meta"><span>${escapeHtml(card.event || 'E-sport')}</span>${card.format ? `<b>${escapeHtml(card.format)}</b>` : ''}${date ? `<b>${escapeHtml(date)}</b>` : ''}</div>${score}${detail}${storeLinks()}<small class="foot">Calls gratuits · Frags = classement · aucun argent réel en jeu.</small></article></main><footer><a href="/">Découvrir Clutch</a><span>CLUTCH // ESPORT CALLS</span></footer></body></html>`;
}

function renderNotFound({ canonical, ogImage }) {
  return `<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Lien introuvable | Clutch</title><meta name="robots" content="noindex,nofollow"><meta property="og:title" content="Clutch"><meta property="og:description" content="Le compagnon compétitif esport."><meta property="og:image" content="${escapeAttr(ogImage)}"><link rel="canonical" href="${escapeAttr(canonical)}"><style>${pageCss()}</style></head><body><header><a class="brand" href="/">CLUTCH<span>.</span></a></header><main><article class="card"><div class="eyebrow">LIEN PUBLIC</div><h1>CE LIEN N’EST PLUS DISPONIBLE.</h1><p>Le match, le défi ou la ligue a peut-être été supprimé ou mal copié.</p><a class="cta" href="/">Découvrir Clutch <span>›</span></a></article></main></body></html>`;
}
